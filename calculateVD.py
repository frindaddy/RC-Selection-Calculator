...
def findBestVoltageDivider(rValues: list[float], targetRatio: float, top: int = 1) -> list[float]:
    """Finds the best resistor combinations to achieve a target voltage divider ratio and displays the top results.

    Args:
        rValues (list[float]): List of resistor values.
        targetRatio (float): Target voltage divider ratio (R2 / (R1 + R2)).
    ...
    """
    ratio = np.array([[r2 / (r1 + r2) for r2 in rValues] for r1 in rValues])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate the best resistor combinations to achieve a target voltage divider ratio.")
    parser.add_argument("ratio", type=float, help="Target voltage divider ratio (e.g. 0.5, 0.25)")
